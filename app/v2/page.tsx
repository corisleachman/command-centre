    ...
    {!loading && <>
      ...
      <section className={styles.todayLowerGrid}>
        <article className={styles.todayMiniCard}>
          <div className={styles.sectionHeading}><div><span className={styles.kicker}>COMMAND CENTRE NOTICED</span><h2>What needs attention</h2></div><span className={styles.signalStatus}>{signalsLoading ? "Checking signals…" : `${proactive.length} ranked signals`}</span></div>
          <div className={styles.proactiveList}>{proactive.length ? proactive.slice(0, 5).map((item, index) => <div key={item.id} className={styles.proactiveRow}><span className={`${styles.recommendationRank} ${styles[item.tone]}`}>{index + 1}</span><div><span className={styles.recommendationSource}>{item.source}</span><strong>{item.title}</strong><small>{item.detail}</small></div><Link href={item.href}>{item.actionLabel} <ArrowRight size={14} /></Link></div>) : <div className={styles.calmState}><CheckCircle2 size={20} /><div><strong>Nothing needs escalation</strong><small>Tasks, Calendar, Gmail and CRM are not producing a stronger intervention right now.</small></div></div>}</div>
        </article>
