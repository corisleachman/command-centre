...
import { createV2Task, loadV2Workspace, setV2TaskComplete, updateV2Task, type V2Task, type V2TaskDraft, type V2Workspace } from "../../../lib/v2-data";
...
export default function TasksPage() {
  ...
  return <main className={styles.page}>
    ...
    {(creating || selected) && <TaskModal task={selected} initiatives={workspace.initiatives} userId={user.id} onClose={() => { setCreating(false); setSelected(null); }} onSaved={async () => { setCreating(false); setSelected(null); await reload(); }} />}
  </main>;
}

function TaskModal({ task, initiatives, userId, onClose, onSaved }: { task: V2Task | null; initiatives: V2Workspace["initiatives"]; userId: string; onClose: () => void; onSaved: () => Promise<void> }) {
  const [draft, setDraft] = useState<V2TaskDraft>({ title: task?.title ?? "", category: task?.category ?? "build", priority: task?.priority ?? 3, estimatedMinutes: task?.estimatedMinutes ?? 30, dueOn: task?.dueOn ?? null, notes: task?.notes ?? "", initiativeId: task?.initiativeId ?? null, milestoneId: task?.milestoneId ?? null });
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
