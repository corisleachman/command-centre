...
const PROSPECTS_URL = Deno.env.get("PROSPECTS_SUPABASE_URL") || "https://paprxejgeepvtbqmfgvt.supabase.co";
const PROSPECTS_SERVICE_ROLE = Deno.env.get("PROSPECTS_SUPABASE_SERVICE_ROLE_KEY") || "";
...
Deno.serve(async request => {
  ...
  try {
    ...
    if (!response.ok) return json({ error: `Prospects CRM request failed (${response.status}).` }, 502);
